begin
    using FHist, Revise, ComponentArrays, Optimization, ForwardDiff, Distributions, CairoMakie, BinnedDistributionFit, Minuit2
    N = Normal(100.0, 5.0)
    N_gaus_truth = 5000
    ...
    prob2 = OptimizationProblem(
        ...
        ub = ComponentArray(norms = [N_total, N_total], p1 = [100.0], p2 = [200.0, 10.0])
    )
    sol = solve(prob2, Optimization.LBFGS(), maxiters = 5000)
    ...
    fig = BinnedDistributionFit.plotthing(NLL, ComponentArray(collect(mi.values), para_axes))
    # @show sol
    # @show para_truth
    # @show fig
end
